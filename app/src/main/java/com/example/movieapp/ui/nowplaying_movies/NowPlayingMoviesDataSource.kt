...
class NowPlayingMoviesDataSource (private var service: TmdbService,
    ...
    override fun loadInitial(
        ...
        scope.launch {
            ...
            try {
                val response=service.getNowPlayingPaged(1)
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, Result>) {
        scope.launch {
            try {
                ...
                val response=service.getNowPlayingPaged(params.key)
                if (response.isSuccessful){
                    val movieList =response.body()?.results
